  ...
  int* score_indices_data)
{
  CUDA_KERNEL_LOOP(idx, nthreads) {
    ...
    int max_expansions = beam_size + bag_size * num_alternatives;
    ...
    if (is_constrained && !beam_complete && timestep >= (num_constraints-1)) {
      ...
      for (int c=0; c<bag_size; ++c) {
        ...
        if (state & (1<<c)) {
          for (int a=0; a<num_alternatives; ++a) {
            int constraint_word = constraint_data[(n*bag_size+c)*num_alternatives+a];
            if (constraint_word == end_of_sequence) {
              continue; // ignore
            }
            unsigned int src_state = state & ~(1<<c); // Src state is same but with zeroed bit c
            int offset = idx*max_expansions + beam_size + c*num_alternatives + a;
  ...
  for (int timestep = 0; timestep < this->sequence_length_; ++timestep) {
    ...
    UpdateConstrainedScore<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        sequence_output_prev, input_sequence_data, bag_size, num_alternatives, 
    ...
    for (int l=0; l<bag_size*num_alternatives; ++l) {
    ...
    for (int l=0; l<this->beam_size_*this->num_states_; ++l) {
      ...
      for (int k=0; k<(this->beam_size_+bag_size*num_alternatives); ++k) {
        os << std::setfill(' ') << std::setw(4) << score_data[l*(this->beam_size_+bag_size*num_alternatives)+k] << " ";
      }
      LOG(INFO) << os.str();
