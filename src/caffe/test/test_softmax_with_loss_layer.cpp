...
class SoftmaxWithLossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  SoftmaxWithLossLayerTest()
    ...
    large_bottom_vec_.push_back(blob_bottom_data_);
    for (int i = 0; i < blob_bottom_label_->count(); ++i) {
      blob_bottom_label_->mutable_cpu_data()[i] = caffe_rng_rand() % 5;
    }
    blob_bottom_vec_.push_back(blob_bottom_label_);
    large_bottom_vec_.push_back(blob_bottom_label_);
    filler.Fill(blob_bottom_weight_);
    large_bottom_vec_.push_back(blob_bottom_weight_);
  ...
  vector<Blob<Dtype>*> large_bottom_vec_;
...
TYPED_TEST(SoftmaxWithLossLayerTest, TestGradientWithWeights) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  //layer_param.add_loss_weight(3);
  SoftmaxWithLossLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-2, 1701);
  checker.CheckGradientExhaustive(&layer, this->large_bottom_vec_,
      this->blob_top_vec_, 0);
}
...
TYPED_TEST(SoftmaxWithLossLayerTest, TestGradientIgnoreLabelWithWeights) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  // labels are in {0, ..., 4}, so we'll ignore about a fifth of them
  layer_param.mutable_loss_param()->set_ignore_label(0);
