...
TYPED_TEST(PermuteLayerTest, TestFoward2D) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PermuteParameter* permute_param = layer_param.mutable_permute_param();
  permute_param->add_order(0);
...
TYPED_TEST(PermuteLayerTest, TestFoward3D) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PermuteParameter* permute_param = layer_param.mutable_permute_param();
  permute_param->add_order(0);
...
TYPED_TEST(PermuteLayerTest, TestTwoPermuteThreeChannel) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PermuteParameter* permute_param = layer_param.mutable_permute_param();
  permute_param->add_order(0);
  ...
  shape.push_back(50);
  shape.push_back(115);
  shape.push_back(2048);
  Blob<Dtype> input1(shape);
  Caffe::set_random_seed(1701);
  // fill the values
  FillerParameter filler_param;
  GaussianFiller<Dtype> filler(filler_param);
  ...
  EXPECT_EQ(output1.shape(0), 50);
  EXPECT_EQ(output1.shape(1), 2048);
  EXPECT_EQ(output1.shape(2), 115);
  ...
  EXPECT_EQ(output2.shape(1), 115);
  EXPECT_EQ(output2.shape(2), 2048);
