...
class BaseBeamSearchLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  // existing beam are first in the output_score_.diff blob. Corresponding word indices are saved in output_score_indices.
  ...
  // Stratified beam search implementations can override to return > 1 as required, ie. how
  // many different sets of beams are required for each image
  virtual int ExactNumStratas(const vector<Blob<Dtype>*>& bottom) { return 1; }
  ...
  int num_stratas_; // How many copies of each beam are needed
  ...
  shared_ptr<Blob<Dtype> > input_sequence_; // Input to next timestep (batch_size * beam_size * num_stratas)
  shared_ptr<Blob<Dtype> > output_score_; // Score output, which is sorted into diff for each beam (batch_size * beam_size * num_stratas, vocab_size)
  Blob<int> output_score_indices_; // Original global index offset associated with sorted output_score_ values (batch_size * beam_size * num_stratas, vocab_size)
  Blob<Dtype> score_; // Scores summed over existing partial sequences (batch_size, beam_size * beam_size * num_stratas)
  Blob<int> score_indices_; // Indices of score_ after sorting (batch_size, beam_size * beam_size * num_stratas)
